...
async function cargarListas() {
    if (!primeraCarga) {
        await guardarJSONEnLocalStorage();
    }
...
function obtenerDatosDeAPI() {
    return new Promise(async (resolve, reject) => {
        try {
            const resp = await fetch('https://api.quotable.io/quotes/random?tags=perseverance|motivational|inspirational|knowledge|life|power-quotes|success|wellness|work');
            if (!resp.ok) {
                throw new Error('Error al obtener datos de la API');
            }
            const datos = await resp.json();
            if (datos && datos[0].content && datos[0].author) {
                const contenidoOriginal = datos[0].content;
                try {
                    const contenidoTraducido = await traducirTexto(contenidoOriginal, 'en', 'es');
                    let mensaje = `"${contenidoTraducido}" - ${datos[0].author}`;
                    resolve(mensaje);
                } catch (error) {
                    console.error('No se pudo traducir el texto. Detalles:', error);
                    let mensaje = `"${contenidoOriginal}" - ${datos[0].author}`;
                    resolve(mensaje);
                }
            } else {
                reject(new Error('Los datos recibidos de la API no son válidos'));
            }
        } catch (error) {
...
async function traducirTexto(texto, idiomaOrigen, idiomaDestino) {
    ...
    myHeaders.append("apikey", "API-KEY-VALUE"); // Usar el valor del api key de la suscripcion
    ...
    try {
        ...
        if (result.translations && result.translations.length > 0) {
            const translation = JSON.parse(result.translations[0].translation);
            const trimmedTranslation = translation.q.trim();
            console.log(trimmedTranslation);
            return trimmedTranslation;
        } else {
            console.log("No se pudo traducir el texto. Devolviendo el texto original.");
            return texto;
        }
    } catch (error) {
...
(async () => {
    try {
        const mensaje = await obtenerDatosDeAPI();
        Swal.fire({
            title: 'Cita del día',
            text: mensaje,
            icon: 'info',
...
document.addEventListener('DOMContentLoaded', function () {
    async function init() {
        ...
        function actualizarListaTareas() {
            ...
            document.getElementById('contenedorTextoTareas').style.display = 'none';
